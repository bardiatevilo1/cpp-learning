...
enum class Category { TEMPERATURE, DISTANCE, MASS };

enum class TempUnit { CELSIUS, FAHRENHEIT, KELVIN };

enum class DistUnit { METRES, KILOMETRES, MILES, FEET };

enum class MassUnit { KILOGRAMS, GRAMS, POUNDS, OUNCES };
...
int main() {
	...
	while (choice == 'y' || choice == 'Y') {
		...
		case Category::TEMPERATURE: {
			...
			int n_t = 0;
			while (n_f == 0 || n_t == 0) {
				std::cout << "Enter Initial units (1. Celsius, 2. Kelvin, 3. "
							 "Fahrenheit)."
						  << std::endl;
				std::cin >> n_f;
				if (std::cin.fail()) {
				...
				std::cout << "Enter Desired units (1. Celsius, 2. Kelvin, 3. "
							 "Fahrenheit)."
						  << std::endl;
				std::cin >> n_t;
				if (std::cin.fail()) {
		...
		case Category::DISTANCE: {
			...
			int n_dt = 0;
			while (n_df == 0 || n_dt == 0) {
				std::cout << "Enter Initial units (1. Metres, 2. Kilometres, "
							 "3. Miles, "
							 "4. Feet)."
						  << std::endl;
				std::cin >> n_df;
				...
				std::cout << "Enter Desired units (1. Metres, 2. Kilometres, "
							 "3. Miles, "
							 "4. Feet)."
						  << std::endl;
				std::cin >> n_dt;
			...
			double dist_result =
				convertDistance(dist_value, dist_from, dist_to);
		...
		case Category::MASS: {
			...
			int n_mt = 0;
			while (n_mf == 0 || n_mt == 0) {
				std::cout << "Enter Initial units (1. Kilograms, 2. Grams, 3. "
							 "Pounds, "
							 "4. Ounces)."
						  << std::endl;
				std::cin >> n_mf;
				...
				std::cout << "Enter Desired units (1. Kilograms, 2. Grams, 3. "
							 "Pounds, "
							 "4. Ounces)."
						  << std::endl;
				std::cin >> n_mt;
...
void displayMenu() {
	...
	std::cout << "=================================================="
			  << std::endl;
	std::cout << '\n';
	std::cout << "What kind of values would you like to convert? (1-3)"
			  << std::endl;
